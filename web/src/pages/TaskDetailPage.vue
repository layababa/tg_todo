...
const assignTask = () => {
  ...
  window.Telegram?.WebApp?.switchInlineQuery?.(
    `share ${task.value.ID} ${task.value.Title}`,
...
const updateDueDate = async () => {
  ...
  const newDate = dueAtLocal.value
    ? new Date(dueAtLocal.value).toISOString()
    : null;
...
const organizedComments = computed(() => {
  ...
  const sortedRaw = [...comments.value].sort(
    (a, b) =>
...
<template>
  <div class="page-root flex flex-col h-full overflow-hidden">
    ...
    >
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <div v-if="!isNew" class="dropdown dropdown-end">
              <button tabindex="0" class="icon-btn tech-btn !border-none">
                <i class="ri-more-2-fill"></i>
              </button>
              <ul
                tabindex="0"
                class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52 border border-base-content/10"
              >
                <li>
                  <a @click="onDelete" class="text-error"
                    ><i class="ri-delete-bin-line"></i> Delete Task</a
                  >
                </li>
              </ul>
            </div>
      ...
      <main class="flex-1 overflow-y-auto px-4 pt-4 pb-4 scroll-smooth">
        ...
        <div v-else-if="task">
          ...
          <div class="grid grid-cols-2 gap-3 mb-8">
            ...
            <div
              @click="cycleStatus"
              class="bg-base-200/50 p-3 rounded-lg border border-base-content/5 relative overflow-hidden group active:scale-95 transition-all cursor-pointer"
            >
              ...
              <div
                ...
                :class="
                  task?.Status === 'Done' ? 'text-success' : 'text-primary'
                "
              >
                {{
              ...
              <div
                class="absolute right-2 top-2 opacity-0 group-hover:opacity-20 transition-opacity"
              >
            ...
            >
              ...
              <div
                ...
                :class="task?.DueAt ? 'text-primary' : 'text-base-content/30'"
              >
              ...
              <div class="flex items-center gap-2">
                ...
                >
                  <div v-for="user in task.Assignees" :key="user.id">
                    ...
                    <div
                      v-else
                      class="w-4 h-4 rounded-full bg-gradient-to-tr from-primary to-secondary flex items-center justify-center text-[8px] text-black font-bold ring-2 ring-base-100/50"
                      :title="user.name"
                    >
                ...
                <div
                  v-else
                  class="w-4 h-4 rounded-full bg-base-content/10 flex items-center justify-center text-[8px] text-base-content/50 font-bold"
                >
                  ?
                </div>
                <span class="font-bold text-sm truncate">
                  {{
                    task?.Assignees && task.Assignees.length > 0
                      ? task.Assignees[0].id === authStore.user?.id
                        ? "我"
            ...
            <div
              class="bg-base-200/50 p-3 rounded-lg border border-base-content/5 relative overflow-hidden opacity-80"
            >
              <div class="flex items-center gap-2 mb-1 opacity-50">
            ...
            <a
              v-if="task?.Creator?.tg_username"
              :href="`https://t.me/${task.Creator.tg_username}`"
              target="_blank"
              class="bg-base-200/50 p-3 rounded-lg border border-base-content/5 relative overflow-hidden group active:scale-95 transition-all cursor-pointer"
            >
              <div class="flex items-center gap-2 mb-1 opacity-50">
              ...
              <div
                class="font-bold text-sm text-primary flex items-center gap-1 truncate"
              >
                <img
                  v-if="task.Creator?.photo_url"
                  :src="task.Creator.photo_url"
                  class="w-4 h-4 rounded-full object-cover mr-1"
                />
                @{{ task.Creator.tg_username }}
                <i class="ri-external-link-line text-xs opacity-50"></i>
              </div>
            </a>
            <div
              v-else
              class="bg-base-200/50 p-3 rounded-lg border border-base-content/5 relative overflow-hidden opacity-50"
            >
              <div class="flex items-center gap-2 mb-1 opacity-50">
                <i class="ri-user-add-line text-xs"></i>
                <span class="text-[10px] uppercase font-mono tracking-wider"
            ...
            <a
              v-if="task?.ChatJumpURL"
              :href="task.ChatJumpURL"
              target="_blank"
              class="bg-[#24A1DE]/10 p-3 rounded-lg border border-[#24A1DE]/20 relative overflow-hidden group active:scale-95 transition-all cursor-pointer"
            >
              ...
              <div
                class="font-bold text-sm text-[#24A1DE] flex items-center gap-1"
              >
                Telegram
                <i class="ri-external-link-line text-xs opacity-50"></i>
              </div>
            ...
            <div
              v-else
              class="bg-base-200/20 p-3 rounded-lg border border-base-content/5 opacity-50"
            >
          ...
          <div v-if="contextItems.length > 0" class="context-snapshot-section">
            <div
              @click="toggleContext"
              class="section-toggle cursor-pointer select-none"
            >
            ...
            <div v-show="contextOpen" class="context-content">
              <div
                v-for="(msg, idx) in contextItems"
                :key="idx"
                :class="['chat-bubble', msg.isMe ? 'me' : 'other']"
              >
          ...
          <div v-if="!isReadyForHeavyContent" class="mt-6 px-4">
            <div
              class="h-24 w-full bg-base-content/10 rounded animate-pulse"
            ></div>
          </div>
          ...
          >
            <div
              class="text-sm font-mono leading-relaxed text-base-content/90 line-clamp-2 whitespace-pre-wrap break-all"
            >
              {{ task.Description }}
            </div>
            <!-- Decorators -->
            <div
              class="absolute top-0 left-0 w-1.5 h-1.5 border-t border-l border-primary/30"
            ></div>
            <div
              class="absolute bottom-0 right-0 w-1.5 h-1.5 border-b border-r border-primary/30"
            ></div>
            <!-- Expand Hint -->
            <div
          ...
          >
            <div
              ...
              class="absolute inset-0 flex items-center px-4 pointer-events-none"
            >
              <span
                class="text-primary/40 font-mono text-sm tracking-wide transform -translate-y-px"
              >
            ...
            <div
              class="absolute bottom-2 right-2 opacity-0 group-focus-within:opacity-100 transition-opacity"
            >
              <span class="text-[10px] font-mono text-primary animate-pulse"
                >自动保存中...</span
              >
            </div>
          </div>
          ...
          <div class="mt-8 border-t border-base-content/10 pt-4 pb-8">
            <div class="flex items-center justify-between mb-4">
              <div
                class="text-[10px] text-primary uppercase font-mono tracking-widest"
              >
                评论
              </div>
              <button
                @click="onFocusComment"
            ...
            <div
              v-if="comments.length === 0 && isReadyForHeavyContent"
              class="text-center py-8 opacity-30 text-xs italic"
            >
              暂无评论
            </div>

            <div
              v-else-if="!isReadyForHeavyContent"
              class="flex flex-col gap-4"
            >
              <!-- Comment Skeleton -->
              <div v-for="i in 2" :key="i" class="flex gap-3">
                <div
                  class="w-8 h-8 rounded-full bg-base-content/10 animate-pulse"
                ></div>
                <div class="flex-1 space-y-2">
                  <div
                    class="h-3 w-1/4 bg-base-content/10 rounded animate-pulse"
                  ></div>
                  <div
                    class="h-10 w-full bg-base-content/10 rounded animate-pulse"
                  ></div>
                </div>
              </div>
            </div>
            ...
            <div v-else class="flex flex-col gap-10">
              <div
                v-for="thread in organizedComments"
                :key="thread.id"
                class="flex flex-col gap-4"
              >
                <CommentItem :comment="thread" @reply="setReply" />
                <div
                  v-if="thread.allReplies.length > 0"
                  class="ml-2 flex flex-col gap-3"
                >
                  <CommentItem
      ...
      <footer
        class="shrink-0 bg-base-100/95 backdrop-blur border-t border-base-content/10 z-40 pb-[env(safe-area-inset-bottom)] transition-all duration-300"
      >
        <!-- Reply Hint -->
        <div
          v-if="replyToCommentID"
          class="px-4 py-2 bg-base-content/5 flex justify-between items-center text-xs font-mono border-b border-base-content/5"
        >
          <span class="text-primary">回复给 @{{ replyToUserName }}</span>
          <button
            @click="cancelReply"
            class="w-5 h-5 flex items-center justify-center rounded-full hover:bg-base-content/10"
          >
        ...
        <div class="p-3 flex items-end gap-3">
          <div
            class="flex-1 bg-base-200/50 rounded-lg border border-base-content/5 focus-within:border-primary/50 focus-within:bg-base-200 transition-all flex items-center"
          >
            <textarea
              v-model="newComment"
              ref="footerTextarea"
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="modal-box bg-base-100 border border-primary/20 shadow-[0_0_50px_rgba(0,0,0,0.5)]"
        >
          <div class="flex justify-between items-center mb-6">
            <h3 class="font-bold text-lg text-primary uppercase">
              设置截止日期
            </h3>
            <button
          ...
          <div class="flex flex-col gap-6">
            <div class="form-control w-full">
              <label class="label"
                ><span class="label-text font-mono text-[10px] uppercase"
                  >选择日期与时间</span
                ></label
              >
              <input
                type="datetime-local"
                v-model="dueAtLocal"
                class="input input-bordered w-full font-mono bg-base-200/50 rounded-none border-base-content/10"
              />
            </div>
            <div class="flex flex-col gap-2">
              ...
              <button
                v-if="task?.DueAt"
                @click="clearDueDate"
                class="btn btn-ghost w-full rounded-none text-xs text-error/60"
              >
                清除截止日期
              </button>
            </div>
