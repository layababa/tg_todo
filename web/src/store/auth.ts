...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    refreshInitData(value?: string) {
      this.initData = resolveInitData(value)
    },
    buildHeaders() {
    ...
    async fetchStatus(startParam?: string) {
      ...
      try {
        ...
        const response = await apiClient.get<AuthStatusPayload>('/auth/status', {
    ...
    async requestNotionAuthUrl(): Promise<string> {
      ...
      this.error = null
      try {
        const response = await apiClient.get<{ url: string }>('/auth/notion/url', {
          headers: this.buildHeaders()
        })
