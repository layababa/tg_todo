...
- **负责人**: AI Agent
- **内容**:
  - **问题**: 在任务详情页与群组设置页，HTML 原生返回按钮与 Telegram Mini App 官方左上角 Back Button 重复显示，造成视觉冗余。
  - **修复**: 移除页面内部的 `返回` 按钮，完全依赖原生的导航栏与左上角返回逻辑（由 `useBackButton` 统一管理）。
  - **影响**: 页面头部更简洁，且导航行为与 Android 硬件后退逻辑保持一致。
- **状态**: ✅ 已完成 (Deployed)
- **修改文件**:
  - `web/src/pages/TaskDetailPage.vue`
  - `web/src/pages/GroupListPage.vue`
