...
| 部署与运维             | `docs/deploy/ops-guide.md`                | SSH、服务管理、数据库操作、清空数据、管理后台           | 生产环境操作、故障排查、DevOps 流程                          |
| **需求池 (Backlog)**   | `docs/backlog/`                           | 验收后的下一迭代需求草案，与当前开发隔离                | ⚠️ **当前迭代不读取**；仅在规划新版本时参考                  |
...
### 6. `docs/deploy/ops-guide.md`

- **生产环境运维指南**：SSH 连接、服务管理、日志查看、数据库操作、清空数据等常用命令。
- **管理后台**：Go-Admin 访问地址、初始化脚本、默认账密。
- **CI/CD**：GitHub Actions 自动部署流程说明。
- **故障排查**：500 错误、数据库连接、Webhook 问题的排查思路。
...
### 9. `docs/backlog/` ⚠️ 隔离目录

- **定位**: 存放验收后下一迭代的需求草案，与当前开发工作完全隔离。
- **命名规范**: `NNN-feature-name.md`（如 `001-chat-based-task-grouping.md`）。
- **使用规则**:
  - ✅ 用户验收时提出的新需求记录于此
  - ✅ 规划新版本时参考
  - ❌ **当前迭代开发时不应读取此目录**（避免 AI 混淆需求边界）
- **合并时机**: 当前迭代完成后，选择性合入 PRD 或创建新版本文档。
