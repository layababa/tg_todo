...
在只使用 Notion 的情况下： 4. 创建效率低：需要离开 Telegram 去 Notion 录入，打断当前讨论 5. 通知不对等：Notion 更新无法及时推送到 Telegram 用户 6. 上下文丢失：任务缺少原始讨论链接，新成员难以补齐背景
...
4.17 故事 S10：日历订阅 (Calendar Sync)

用户希望在手机系统日历中直接查看截止日期临近的待办事项

在 Mini App「个人设置」中点击「添加日历订阅」

系统生成一个专属的 Webcal 链接 (如 `webcal://api.site/cal/xyz.ics`)

系统唤起手机日历应用（如 iOS Calendar），请求订阅授权

订阅成功后，所有分配给该用户的未完成任务（且带有 Due Date）会自动同步显示在日历视图中

若用户更新了任务时间，日历会自动同步变更（依赖系统刷新频率）
...
Notion 变更 → Telegram 通知延迟 < 2 分钟 (受限于轮询频率)
...
Q9：如果 Notion 的 API 限流了怎么办？（Rate Limiting） A：系统需实现指数退避算法 (Exponential Backoff)。当触发 429 错误时，Bot 会暂停该 Database 的同步队列，并延迟重试。对于高频群组，建议适当降低轮询频率（如从 1 分钟降为 5 分钟）。
